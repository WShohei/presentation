...
#let split_slide(title: "", content_left, content_right, left_ratio: 1fr, right_ratio: 1fr) = locate(loc => {
	// Header with slide title.
	let header = {
		let headers = query(selector(heading).before(loc), loc)
		set align(left + top)
	...
	grid(rows: (auto, 1fr, auto), {
		...
		// Inner slide content split left and right.
		pad(x: 10pt, y: 10pt, grid(columns: (left_ratio, right_ratio), {
			box(align(left, content_left))
		}, {
			box(align(left, content_right))
		}))
	}, {
		footer
	})
